export const countryList = [
    {
        label: 'Afghanistan',
        dialCode: '+93',
        value: 'AF',
    },
    {
        label: 'Aland Islands',
        dialCode: '+358',
        value: 'AX',
    },
    {
        label: 'Albania',
        dialCode: '+355',
        value: 'AL',
    },
    {
        label: 'Algeria',
        dialCode: '+213',
        value: 'DZ',
    },
    {
        label: 'AmericanSamoa',
        dialCode: '+1684',
        value: 'AS',
    },
    {
        label: 'Andorra',
        dialCode: '+376',
        value: 'AD',
    },
    {
        label: 'Angola',
        dialCode: '+244',
        value: 'AO',
    },
    {
        label: 'Anguilla',
        dialCode: '+1264',
        value: 'AI',
    },
    {
        label: 'Antarctica',
        dialCode: '+672',
        value: 'AQ',
    },
    {
        label: 'Antigua and Barbuda',
        dialCode: '+1268',
        value: 'AG',
    },
    {
        label: 'Argentina',
        dialCode: '+54',
        value: 'AR',
    },
    {
        label: 'Armenia',
        dialCode: '+374',
        value: 'AM',
    },
    {
        label: 'Aruba',
        dialCode: '+297',
        value: 'AW',
    },
    {
        label: 'Australia',
        dialCode: '+61',
        value: 'AU',
    },
    {
        label: 'Austria',
        dialCode: '+43',
        value: 'AT',
    },
    {
        label: 'Azerbaijan',
        dialCode: '+994',
        value: 'AZ',
    },
    {
        label: 'Bahamas',
        dialCode: '+1242',
        value: 'BS',
    },
    {
        label: 'Bahrain',
        dialCode: '+973',
        value: 'BH',
    },
    {
        label: 'Bangladesh',
        dialCode: '+880',
        value: 'BD',
    },
    {
        label: 'Barbados',
        dialCode: '+1246',
        value: 'BB',
    },
    {
        label: 'Belarus',
        dialCode: '+375',
        value: 'BY',
    },
    {
        label: 'Belgium',
        dialCode: '+32',
        value: 'BE',
    },
    {
        label: 'Belize',
        dialCode: '+501',
        value: 'BZ',
    },
    {
        label: 'Benin',
        dialCode: '+229',
        value: 'BJ',
    },
    {
        label: 'Bermuda',
        dialCode: '+1441',
        value: 'BM',
    },
    {
        label: 'Bhutan',
        dialCode: '+975',
        value: 'BT',
    },
    {
        label: 'Bolivia, Plurinational State of',
        dialCode: '+591',
        value: 'BO',
    },
    {
        label: 'Bosnia and Herzegovina',
        dialCode: '+387',
        value: 'BA',
    },
    {
        label: 'Botswana',
        dialCode: '+267',
        value: 'BW',
    },
    {
        label: 'Brazil',
        dialCode: '+55',
        value: 'BR',
    },
    {
        label: 'British Indian Ocean Territory',
        dialCode: '+246',
        value: 'IO',
    },
    {
        label: 'Brunei Darussalam',
        dialCode: '+673',
        value: 'BN',
    },
    {
        label: 'Bulgaria',
        dialCode: '+359',
        value: 'BG',
    },
    {
        label: 'Burkina Faso',
        dialCode: '+226',
        value: 'BF',
    },
    {
        label: 'Burundi',
        dialCode: '+257',
        value: 'BI',
    },
    {
        label: 'Cambodia',
        dialCode: '+855',
        value: 'KH',
    },
    {
        label: 'Cameroon',
        dialCode: '+237',
        value: 'CM',
    },
    {
        label: 'Canada',
        dialCode: '+1 ',
        value: 'CA',
    },
    {
        label: 'Cape Verde',
        dialCode: '+238',
        value: 'CV',
    },
    {
        label: 'Cayman Islands',
        dialCode: '+ 345',
        value: 'KY',
    },
    {
        label: 'Central African Republic',
        dialCode: '+236',
        value: 'CF',
    },
    {
        label: 'Chad',
        dialCode: '+235',
        value: 'TD',
    },
    {
        label: 'Chile',
        dialCode: '+56',
        value: 'CL',
    },
    {
        label: 'China',
        dialCode: '+86',
        value: 'CN',
    },
    {
        label: 'Christmas Island',
        dialCode: '+61',
        value: 'CX',
    },
    {
        label: 'Cocos (Keeling) Islands',
        dialCode: '+61',
        value: 'CC',
    },
    {
        label: 'Colombia',
        dialCode: '+57',
        value: 'CO',
    },
    {
        label: 'Comoros',
        dialCode: '+269',
        value: 'KM',
    },
    {
        label: 'Congo',
        dialCode: '+242',
        value: 'CG',
    },
    {
        label: 'Congo, The Democratic Republic of the Congo',
        dialCode: '+243',
        value: 'CD',
    },
    {
        label: 'Cook Islands',
        dialCode: '+682',
        value: 'CK',
    },
    {
        label: 'Costa Rica',
        dialCode: '+506',
        value: 'CR',
    },
    {
        label: "Cote d'Ivoire",
        dialCode: '+225',
        value: 'CI',
    },
    {
        label: 'Croatia',
        dialCode: '+385',
        value: 'HR',
    },
    {
        label: 'Cuba',
        dialCode: '+53',
        value: 'CU',
    },
    {
        label: 'Cyprus',
        dialCode: '+357',
        value: 'CY',
    },
    {
        label: 'Czech Republic',
        dialCode: '+420',
        value: 'CZ',
    },
    {
        label: 'Denmark',
        dialCode: '+45',
        value: 'DK',
    },
    {
        label: 'Djibouti',
        dialCode: '+253',
        value: 'DJ',
    },
    {
        label: 'Dominica',
        dialCode: '+1767',
        value: 'DM',
    },
    {
        label: 'Dominican Republic',
        dialCode: '+1849',
        value: 'DO',
    },
    {
        label: 'Ecuador',
        dialCode: '+593',
        value: 'EC',
    },
    {
        label: 'Egypt',
        dialCode: '+20',
        value: 'EG',
    },
    {
        label: 'El Salvador',
        dialCode: '+503',
        value: 'SV',
    },
    {
        label: 'Equatorial Guinea',
        dialCode: '+240',
        value: 'GQ',
    },
    {
        label: 'Eritrea',
        dialCode: '+291',
        value: 'ER',
    },
    {
        label: 'Estonia',
        dialCode: '+372',
        value: 'EE',
    },
    {
        label: 'Ethiopia',
        dialCode: '+251',
        value: 'ET',
    },
    {
        label: 'Falkland Islands (Malvinas)',
        dialCode: '+500',
        value: 'FK',
    },
    {
        label: 'Faroe Islands',
        dialCode: '+298',
        value: 'FO',
    },
    {
        label: 'Fiji',
        dialCode: '+679',
        value: 'FJ',
    },
    {
        label: 'Finland',
        dialCode: '+358',
        value: 'FI',
    },
    {
        label: 'France',
        dialCode: '+33',
        value: 'FR',
    },
    {
        label: 'French Polynesia',
        dialCode: '+689',
        value: 'PF',
    },
    {
        label: 'Gabon',
        dialCode: '+241',
        value: 'GA',
    },
    {
        label: 'Gambia',
        dialCode: '+220',
        value: 'GM',
    },
    {
        label: 'Georgia',
        dialCode: '+995',
        value: 'GE',
    },
    {
        label: 'Germany',
        dialCode: '+49',
        value: 'DE',
    },
    {
        label: 'Ghana',
        dialCode: '+233',
        value: 'GH',
    },
    {
        label: 'Gibraltar',
        dialCode: '+350',
        value: 'GI',
    },
    {
        label: 'Greece',
        dialCode: '+30',
        value: 'GR',
    },
    {
        label: 'Greenland',
        dialCode: '+299',
        value: 'GL',
    },
    {
        label: 'Grenada',
        dialCode: '+1473',
        value: 'GD',
    },
    {
        label: 'Guam',
        dialCode: '+1671',
        value: 'GU',
    },
    {
        label: 'Guatemala',
        dialCode: '+502',
        value: 'GT',
    },
    {
        label: 'Guernsey',
        dialCode: '+44',
        value: 'GG',
    },
    {
        label: 'Guinea',
        dialCode: '+224',
        value: 'GN',
    },
    {
        label: 'Guinea-Bissau',
        dialCode: '+245',
        value: 'GW',
    },
    {
        label: 'Guyana',
        dialCode: '+595',
        value: 'GY',
    },
    {
        label: 'Haiti',
        dialCode: '+509',
        value: 'HT',
    },
    {
        label: 'Holy See (Vatican City State)',
        dialCode: '+379',
        value: 'VA',
    },
    {
        label: 'Honduras',
        dialCode: '+504',
        value: 'HN',
    },
    {
        label: 'Hong Kong',
        dialCode: '+852',
        value: 'HK',
    },
    {
        label: 'Hungary',
        dialCode: '+36',
        value: 'HU',
    },
    {
        label: 'Iceland',
        dialCode: '+354',
        value: 'IS',
    },
    {
        label: 'India',
        dialCode: '+91',
        value: 'IN',
    },
    {
        label: 'Indonesia',
        dialCode: '+62',
        value: 'ID',
    },
    {
        label: 'Iran, Islamic Republic of Persian Gulf',
        dialCode: '+98',
        value: 'IR',
    },
    {
        label: 'Iraq',
        dialCode: '+964',
        value: 'IQ',
    },
    {
        label: 'Ireland',
        dialCode: '+353',
        value: 'IE',
    },
    {
        label: 'Isle of Man',
        dialCode: '+44',
        value: 'IM',
    },
    {
        label: 'Israel',
        dialCode: '+972',
        value: 'IL',
    },
    {
        label: 'Italy',
        dialCode: '+39',
        value: 'IT',
    },
    {
        label: 'Jamaica',
        dialCode: '+1876',
        value: 'JM',
    },
    {
        label: 'Japan',
        dialCode: '+81',
        value: 'JP',
    },
    {
        label: 'Jersey',
        dialCode: '+44',
        value: 'JE',
    },
    {
        label: 'Jordan',
        dialCode: '+962',
        value: 'JO',
    },
    {
        label: 'Kazakhstan',
        dialCode: '+77',
        value: 'KZ',
    },
    {
        label: 'Kenya',
        dialCode: '+254',
        value: 'KE',
    },
    {
        label: 'Kiribati',
        dialCode: '+686',
        value: 'KI',
    },
    {
        label: "Korea, Democratic People's Republic of Korea",
        dialCode: '+850',
        value: 'KP',
    },
    {
        label: 'Korea, Republic of South Korea',
        dialCode: '+82',
        value: 'KR',
    },
    {
        label: 'Kuwait',
        dialCode: '+965',
        value: 'KW',
    },
    {
        label: 'Kyrgyzstan',
        dialCode: '+996',
        value: 'KG',
    },
    {
        label: 'Laos',
        dialCode: '+856',
        value: 'LA',
    },
    {
        label: 'Latvia',
        dialCode: '+371',
        value: 'LV',
    },
    {
        label: 'Lebanon',
        dialCode: '+961',
        value: 'LB',
    },
    {
        label: 'Lesotho',
        dialCode: '+266',
        value: 'LS',
    },
    {
        label: 'Liberia',
        dialCode: '+231',
        value: 'LR',
    },
    {
        label: 'Libyan Arab Jamahiriya',
        dialCode: '+218',
        value: 'LY',
    },
    {
        label: 'Liechtenstein',
        dialCode: '+423',
        value: 'LI',
    },
    {
        label: 'Lithuania',
        dialCode: '+370',
        value: 'LT',
    },
    {
        label: 'Luxembourg',
        dialCode: '+352',
        value: 'LU',
    },
    {
        label: 'Macao',
        dialCode: '+853',
        value: 'MO',
    },
    {
        label: 'Macedonia',
        dialCode: '+389',
        value: 'MK',
    },
    {
        label: 'Madagascar',
        dialCode: '+261',
        value: 'MG',
    },
    {
        label: 'Malawi',
        dialCode: '+265',
        value: 'MW',
    },
    {
        label: 'Malaysia',
        dialCode: '+60',
        value: 'MY',
    },
    {
        label: 'Maldives',
        dialCode: '+960',
        value: 'MV',
    },
    {
        label: 'Mali',
        dialCode: '+223',
        value: 'ML',
    },
    {
        label: 'Malta',
        dialCode: '+356',
        value: 'MT',
    },
    {
        label: 'Marshall Islands',
        dialCode: '+692',
        value: 'MH',
    },
    {
        label: 'Martinique',
        dialCode: '+596',
        value: 'MQ',
    },
    {
        label: 'Mauritania',
        dialCode: '+222',
        value: 'MR',
    },
    {
        label: 'Mauritius',
        dialCode: '+230',
        value: 'MU',
    },
    {
        label: 'Mexico',
        dialCode: '+52',
        value: 'MX',
    },
    {
        label: 'Micronesia, Federated States of Micronesia',
        dialCode: '+691',
        value: 'FM',
    },
    {
        label: 'Moldova',
        dialCode: '+373',
        value: 'MD',
    },
    {
        label: 'Monaco',
        dialCode: '+377',
        value: 'MC',
    },
    {
        label: 'Mongolia',
        dialCode: '+976',
        value: 'MN',
    },
    {
        label: 'Montenegro',
        dialCode: '+382',
        value: 'ME',
    },
    {
        label: 'Montserrat',
        dialCode: '+1664',
        value: 'MS',
    },
    {
        label: 'Morocco',
        dialCode: '+212',
        value: 'MA',
    },
    {
        label: 'Mozambique',
        dialCode: '+258',
        value: 'MZ',
    },
    {
        label: 'Myanmar',
        dialCode: '+95',
        value: 'MM',
    },
    {
        label: 'Namibia',
        dialCode: '+264',
        value: 'NA',
    },
    {
        label: 'Nauru',
        dialCode: '+674',
        value: 'NR',
    },
    {
        label: 'Nepal',
        dialCode: '+977',
        value: 'NP',
    },
    {
        label: 'Netherlands',
        dialCode: '+31',
        value: 'NL',
    },
    {
        label: 'New Zealand',
        dialCode: '+64',
        value: 'NZ',
    },
    {
        label: 'Nicaragua',
        dialCode: '+505',
        value: 'NI',
    },
    {
        label: 'Niger',
        dialCode: '+227',
        value: 'NE',
    },
    {
        label: 'Nigeria',
        dialCode: '+234',
        value: 'NG',
    },
    {
        label: 'Niue',
        dialCode: '+683',
        value: 'NU',
    },
    {
        label: 'Norfolk Island',
        dialCode: '+672',
        value: 'NF',
    },
    {
        label: 'Northern Mariana Islands',
        dialCode: '+1670',
        value: 'MP',
    },
    {
        label: 'Norway',
        dialCode: '+47',
        value: 'NO',
    },
    {
        label: 'Oman',
        dialCode: '+968',
        value: 'OM',
    },
    {
        label: 'Pakistan',
        dialCode: '+92',
        value: 'PK',
    },
    {
        label: 'Palau',
        dialCode: '+680',
        value: 'PW',
    },
    {
        label: 'Palestinian Territory, Occupied',
        dialCode: '+970',
        value: 'PS',
    },
    {
        label: 'Panama',
        dialCode: '+507',
        value: 'PA',
    },
    {
        label: 'Papua New Guinea',
        dialCode: '+675',
        value: 'PG',
    },
    {
        label: 'Paraguay',
        dialCode: '+595',
        value: 'PY',
    },
    {
        label: 'Peru',
        dialCode: '+51',
        value: 'PE',
    },
    {
        label: 'Philippines',
        dialCode: '+63',
        value: 'PH',
    },
    {
        label: 'Pitcairn',
        dialCode: '+872',
        value: 'PN',
    },
    {
        label: 'Poland',
        dialCode: '+48',
        value: 'PL',
    },
    {
        label: 'Portugal',
        dialCode: '+351',
        value: 'PT',
    },
    {
        label: 'Puerto Rico',
        dialCode: '+1939',
        value: 'PR',
    },
    {
        label: 'Qatar',
        dialCode: '+974',
        value: 'QA',
    },
    {
        label: 'Romania',
        dialCode: '+40',
        value: 'RO',
    },
    {
        label: 'Russia',
        dialCode: '+7',
        value: 'RU',
    },
    {
        label: 'Rwanda',
        dialCode: '+250',
        value: 'RW',
    },
    {
        label: 'Saint Barthelemy',
        dialCode: '+590',
        value: 'BL',
    },
    {
        label: 'Saint Kitts and Nevis',
        dialCode: '+1869',
        value: 'KN',
    },
    {
        label: 'Saint Lucia',
        dialCode: '+1758',
        value: 'LC',
    },
    {
        label: 'Saint Vincent and the Grenadines',
        dialCode: '+1784',
        value: 'VC',
    },
    {
        label: 'Samoa',
        dialCode: '+685',
        value: 'WS',
    },
    {
        label: 'San Marino',
        dialCode: '+378',
        value: 'SM',
    },
    {
        label: 'Sao Tome and Principe',
        dialCode: '+239',
        value: 'ST',
    },
    {
        label: 'Saudi Arabia',
        dialCode: '+966',
        value: 'SA',
    },
    {
        label: 'Senegal',
        dialCode: '+221',
        value: 'SN',
    },
    {
        label: 'Serbia',
        dialCode: '+381',
        value: 'RS',
    },
    {
        label: 'Seychelles',
        dialCode: '+248',
        value: 'SC',
    },
    {
        label: 'Sierra Leone',
        dialCode: '+232',
        value: 'SL',
    },
    {
        label: 'Singapore',
        dialCode: '+65',
        value: 'SG',
    },
    {
        label: 'Slovakia',
        dialCode: '+421',
        value: 'SK',
    },
    {
        label: 'Slovenia',
        dialCode: '+386',
        value: 'SI',
    },
    {
        label: 'Solomon Islands',
        dialCode: '+677',
        value: 'SB',
    },
    {
        label: 'Somalia',
        dialCode: '+252',
        value: 'SO',
    },
    {
        label: 'South Africa',
        dialCode: '+27',
        value: 'ZA',
    },
    {
        label: 'South Sudan',
        dialCode: '+211',
        value: 'SS',
    },
    {
        label: 'Spain',
        dialCode: '+34',
        value: 'ES',
    },
    {
        label: 'Sri Lanka',
        dialCode: '+94',
        value: 'LK',
    },
    {
        label: 'Sudan',
        dialCode: '+249',
        value: 'SD',
    },
    {
        label: 'Suriname',
        dialCode: '+597',
        value: 'SR',
    },
    {
        label: 'Swaziland',
        dialCode: '+268',
        value: 'SZ',
    },
    {
        label: 'Sweden',
        dialCode: '+46',
        value: 'SE',
    },
    {
        label: 'Switzerland',
        dialCode: '+41',
        value: 'CH',
    },
    {
        label: 'Syrian Arab Republic',
        dialCode: '+963',
        value: 'SY',
    },
    {
        label: 'Taiwan',
        dialCode: '+886',
        value: 'TW',
    },
    {
        label: 'Tajikistan',
        dialCode: '+992',
        value: 'TJ',
    },
    {
        label: 'Tanzania, United Republic of Tanzania',
        dialCode: '+255',
        value: 'TZ',
    },
    {
        label: 'Thailand',
        dialCode: '+66',
        value: 'TH',
    },
    {
        label: 'Timor-Leste',
        dialCode: '+670',
        value: 'TL',
    },
    {
        label: 'Togo',
        dialCode: '+228',
        value: 'TG',
    },
    {
        label: 'Tokelau',
        dialCode: '+690',
        value: 'TK',
    },
    {
        label: 'Tonga',
        dialCode: '+676',
        value: 'TO',
    },
    {
        label: 'Trinidad and Tobago',
        dialCode: '+1868',
        value: 'TT',
    },
    {
        label: 'Tunisia',
        dialCode: '+216',
        value: 'TN',
    },
    {
        label: 'Turkey',
        dialCode: '+90',
        value: 'TR',
    },
    {
        label: 'Turkmenistan',
        dialCode: '+993',
        value: 'TM',
    },
    {
        label: 'Turks and Caicos Islands',
        dialCode: '+1649',
        value: 'TC',
    },
    {
        label: 'Tuvalu',
        dialCode: '+688',
        value: 'TV',
    },
    {
        label: 'Uganda',
        dialCode: '+256',
        value: 'UG',
    },
    {
        label: 'Ukraine',
        dialCode: '+380',
        value: 'UA',
    },
    {
        label: 'United Arab Emirates',
        dialCode: '+971',
        value: 'AE',
    },
    {
        label: 'United Kingdom',
        dialCode: '+44',
        value: 'GB',
    },
    {
        label: 'United States',
        dialCode: '+1',
        value: 'US',
    },
    {
        label: 'Uruguay',
        dialCode: '+598',
        value: 'UY',
    },
    {
        label: 'Uzbekistan',
        dialCode: '+998',
        value: 'UZ',
    },
    {
        label: 'Vanuatu',
        dialCode: '+678',
        value: 'VU',
    },
    {
        label: 'Venezuela, Bolivarian Republic of Venezuela',
        dialCode: '+58',
        value: 'VE',
    },
    {
        label: 'Vietnam',
        dialCode: '+84',
        value: 'VN',
    },
    {
        label: 'Virgin Islands, British',
        dialCode: '+1284',
        value: 'VG',
    },
    {
        label: 'Virgin Islands, U.S.',
        dialCode: '+1340',
        value: 'VI',
    },
    {
        label: 'Yemen',
        dialCode: '+967',
        value: 'YE',
    },
    {
        label: 'Zambia',
        dialCode: '+260',
        value: 'ZM',
    },
    {
        label: 'Zimbabwe',
        dialCode: '+263',
        value: 'ZW',
    },
]

export const timezoneList = [
    { label: '(UTC -11:00) Pacific/Midway', value: 'Pacific/Midway' },
    { label: '(UTC -11:00) Pacific/Niue', value: 'Pacific/Niue' },
    { label: '(UTC -11:00) Pacific/Pago_Pago', value: 'Pacific/Pago_Pago' },
    { label: '(UTC -10:00) America/Adak', value: 'America/Adak' },
    { label: '(UTC -10:00) Pacific/Honolulu', value: 'Pacific/Honolulu' },
    { label: '(UTC -10:00) Pacific/Rarotonga', value: 'Pacific/Rarotonga' },
    { label: '(UTC -10:00) Pacific/Tahiti', value: 'Pacific/Tahiti' },
    { label: '(UTC -09:30) Pacific/Marquesas', value: 'Pacific/Marquesas' },
    { label: '(UTC -09:00) America/Anchorage', value: 'America/Anchorage' },
    { label: '(UTC -09:00) America/Juneau', value: 'America/Juneau' },
    { label: '(UTC -09:00) America/Metlakatla', value: 'America/Metlakatla' },
    { label: '(UTC -09:00) America/Nome', value: 'America/Nome' },
    { label: '(UTC -09:00) America/Sitka', value: 'America/Sitka' },
    { label: '(UTC -09:00) America/Yakutat', value: 'America/Yakutat' },
    { label: '(UTC -09:00) Pacific/Gambier', value: 'Pacific/Gambier' },
    { label: '(UTC -08:00) America/Dawson', value: 'America/Dawson' },
    { label: '(UTC -08:00) America/Los_Angeles', value: 'America/Los_Angeles' },
    { label: '(UTC -08:00) America/Tijuana', value: 'America/Tijuana' },
    { label: '(UTC -08:00) America/Vancouver', value: 'America/Vancouver' },
    { label: '(UTC -08:00) America/Whitehorse', value: 'America/Whitehorse' },
    { label: '(UTC -08:00) Pacific/Pitcairn', value: 'Pacific/Pitcairn' },
    { label: '(UTC -07:00) America/Boise', value: 'America/Boise' },
    {
        label: '(UTC -07:00) America/Cambridge_Bay',
        value: 'America/Cambridge_Bay',
    },
    { label: '(UTC -07:00) America/Chihuahua', value: 'America/Chihuahua' },
    { label: '(UTC -07:00) America/Creston', value: 'America/Creston' },
    {
        label: '(UTC -07:00) America/Dawson_Creek',
        value: 'America/Dawson_Creek',
    },
    { label: '(UTC -07:00) America/Denver', value: 'America/Denver' },
    { label: '(UTC -07:00) America/Edmonton', value: 'America/Edmonton' },
    { label: '(UTC -07:00) America/Fort_Nelson', value: 'America/Fort_Nelson' },
    { label: '(UTC -07:00) America/Hermosillo', value: 'America/Hermosillo' },
    { label: '(UTC -07:00) America/Inuvik', value: 'America/Inuvik' },
    { label: '(UTC -07:00) America/Mazatlan', value: 'America/Mazatlan' },
    { label: '(UTC -07:00) America/Ojinaga', value: 'America/Ojinaga' },
    { label: '(UTC -07:00) America/Phoenix', value: 'America/Phoenix' },
    { label: '(UTC -07:00) America/Yellowknife', value: 'America/Yellowknife' },
    {
        label: '(UTC -06:00) America/Bahia_Banderas',
        value: 'America/Bahia_Banderas',
    },
    { label: '(UTC -06:00) America/Belize', value: 'America/Belize' },
    { label: '(UTC -06:00) America/Chicago', value: 'America/Chicago' },
    { label: '(UTC -06:00) America/Costa_Rica', value: 'America/Costa_Rica' },
    { label: '(UTC -06:00) America/El_Salvador', value: 'America/El_Salvador' },
    { label: '(UTC -06:00) America/Guatemala', value: 'America/Guatemala' },
    {
        label: '(UTC -06:00) America/Indiana/Knox',
        value: 'America/Indiana/Knox',
    },
    {
        label: '(UTC -06:00) America/Indiana/Tell_City',
        value: 'America/Indiana/Tell_City',
    },
    { label: '(UTC -06:00) America/Managua', value: 'America/Managua' },
    { label: '(UTC -06:00) America/Matamoros', value: 'America/Matamoros' },
    { label: '(UTC -06:00) America/Menominee', value: 'America/Menominee' },
    { label: '(UTC -06:00) America/Merida', value: 'America/Merida' },
    { label: '(UTC -06:00) America/Mexico_City', value: 'America/Mexico_City' },
    { label: '(UTC -06:00) America/Monterrey', value: 'America/Monterrey' },
    {
        label: '(UTC -06:00) America/North_Dakota/Beulah',
        value: 'America/North_Dakota/Beulah',
    },
    {
        label: '(UTC -06:00) America/North_Dakota/Center',
        value: 'America/North_Dakota/Center',
    },
    {
        label: '(UTC -06:00) America/North_Dakota/New_Salem',
        value: 'America/North_Dakota/New_Salem',
    },
    { label: '(UTC -06:00) America/Rainy_River', value: 'America/Rainy_River' },
    {
        label: '(UTC -06:00) America/Rankin_Inlet',
        value: 'America/Rankin_Inlet',
    },
    { label: '(UTC -06:00) America/Regina', value: 'America/Regina' },
    { label: '(UTC -06:00) America/Regina', value: 'America/Regina' },
    { label: '(UTC -06:00) America/Resolute', value: 'America/Resolute' },
    {
        label: '(UTC -06:00) America/Swift_Current',
        value: 'America/Swift_Current',
    },
    { label: '(UTC -06:00) America/Tegucigalpa', value: 'America/Tegucigalpa' },
    { label: '(UTC -06:00) America/Winnipeg', value: 'America/Winnipeg' },
    { label: '(UTC -06:00) Pacific/Galapagos', value: 'Pacific/Galapagos' },
    { label: '(UTC -05:00) America/Atikokan', value: 'America/Atikokan' },
    { label: '(UTC -05:00) America/Bogota', value: 'America/Bogota' },
    { label: '(UTC -05:00) America/Cancun', value: 'America/Cancun' },
    { label: '(UTC -05:00) America/Cayman', value: 'America/Cayman' },
    { label: '(UTC -05:00) America/Detroit', value: 'America/Detroit' },
    { label: '(UTC -05:00) America/Eirunepe', value: 'America/Eirunepe' },
    { label: '(UTC -05:00) America/Guayaquil', value: 'America/Guayaquil' },
    { label: '(UTC -05:00) America/Havana', value: 'America/Havana' },
    {
        label: '(UTC -05:00) America/Indiana/Indianapolis',
        value: 'America/Indiana/Indianapolis',
    },
    {
        label: '(UTC -05:00) America/Indiana/Marengo',
        value: 'America/Indiana/Marengo',
    },
    {
        label: '(UTC -05:00) America/Indiana/Petersburg',
        value: 'America/Indiana/Petersburg',
    },
    {
        label: '(UTC -05:00) America/Indiana/Vevay',
        value: 'America/Indiana/Vevay',
    },
    {
        label: '(UTC -05:00) America/Indiana/Vincennes',
        value: 'America/Indiana/Vincennes',
    },
    {
        label: '(UTC -05:00) America/Indiana/Winamac',
        value: 'America/Indiana/Winamac',
    },
    { label: '(UTC -05:00) America/Iqaluit', value: 'America/Iqaluit' },
    { label: '(UTC -05:00) America/Jamaica', value: 'America/Jamaica' },
    {
        label: '(UTC -05:00) America/Kentucky/Louisville',
        value: 'America/Kentucky/Louisville',
    },
    {
        label: '(UTC -05:00) America/Kentucky/Monticello',
        value: 'America/Kentucky/Monticello',
    },
    { label: '(UTC -05:00) America/Lima', value: 'America/Lima' },
    { label: '(UTC -05:00) America/Nassau', value: 'America/Nassau' },
    { label: '(UTC -05:00) America/New_York', value: 'America/New_York' },
    { label: '(UTC -05:00) America/Nipigon', value: 'America/Nipigon' },
    { label: '(UTC -05:00) America/Panama', value: 'America/Panama' },
    { label: '(UTC -05:00) America/Pangnirtung', value: 'America/Pangnirtung' },
    {
        label: '(UTC -05:00) America/Port-au-Prince',
        value: 'America/Port-au-Prince',
    },
    { label: '(UTC -05:00) America/Rio_Branco', value: 'America/Rio_Branco' },
    { label: '(UTC -05:00) America/Thunder_Bay', value: 'America/Thunder_Bay' },
    { label: '(UTC -05:00) America/Toronto', value: 'America/Toronto' },
    { label: '(UTC -05:00) Pacific/Easter', value: 'Pacific/Easter' },
    { label: '(UTC -04:00) America/Anguilla', value: 'America/Anguilla' },
    { label: '(UTC -04:00) America/Antigua', value: 'America/Antigua' },
    { label: '(UTC -04:00) America/Aruba', value: 'America/Aruba' },
    { label: '(UTC -04:00) America/Barbados', value: 'America/Barbados' },
    {
        label: '(UTC -04:00) America/Blanc-Sablon',
        value: 'America/Blanc-Sablon',
    },
    { label: '(UTC -04:00) America/Boa_Vista', value: 'America/Boa_Vista' },
    { label: '(UTC -04:00) America/Caracas', value: 'America/Caracas' },
    { label: '(UTC -04:00) America/Curacao', value: 'America/Curacao' },
    { label: '(UTC -04:00) America/Dominica', value: 'America/Dominica' },
    { label: '(UTC -04:00) America/Glace_Bay', value: 'America/Glace_Bay' },
    { label: '(UTC -04:00) America/Goose_Bay', value: 'America/Goose_Bay' },
    { label: '(UTC -04:00) America/Grand_Turk', value: 'America/Grand_Turk' },
    { label: '(UTC -04:00) America/Grenada', value: 'America/Grenada' },
    { label: '(UTC -04:00) America/Guadeloupe', value: 'America/Guadeloupe' },
    { label: '(UTC -04:00) America/Guyana', value: 'America/Guyana' },
    { label: '(UTC -04:00) America/Halifax', value: 'America/Halifax' },
    { label: '(UTC -04:00) America/Kralendijk', value: 'America/Kralendijk' },
    { label: '(UTC -04:00) America/La_Paz', value: 'America/La_Paz' },
    {
        label: '(UTC -04:00) America/Lower_Princes',
        value: 'America/Lower_Princes',
    },
    { label: '(UTC -04:00) America/Manaus', value: 'America/Manaus' },
    { label: '(UTC -04:00) America/Marigot', value: 'America/Marigot' },
    { label: '(UTC -04:00) America/Martinique', value: 'America/Martinique' },
    { label: '(UTC -04:00) America/Moncton', value: 'America/Moncton' },
    { label: '(UTC -04:00) America/Montserrat', value: 'America/Montserrat' },
    {
        label: '(UTC -04:00) America/Port_of_Spain',
        value: 'America/Port_of_Spain',
    },
    {
        label: '(UTC -04:00) America/Porto_Velho',
        value: 'America/Porto_Velho',
    },
    {
        label: '(UTC -04:00) America/Puerto_Rico',
        value: 'America/Puerto_Rico',
    },
    {
        label: '(UTC -04:00) America/Santo_Domingo',
        value: 'America/Santo_Domingo',
    },
    {
        label: '(UTC -04:00) America/St_Barthelemy',
        value: 'America/St_Barthelemy',
    },
    {
        label: '(UTC -04:00) America/St_Kitts',
        value: 'America/St_Kitts',
    },
    {
        label: '(UTC -04:00) America/St_Lucia',
        value: 'America/St_Lucia',
    },
    {
        label: '(UTC -04:00) America/St_Thomas',
        value: 'America/St_Thomas',
    },
    {
        label: '(UTC -04:00) America/St_Vincent',
        value: 'America/St_Vincent',
    },
    {
        label: '(UTC -04:00) America/Thule',
        value: 'America/Thule',
    },
    {
        label: '(UTC -04:00) America/Tortola',
        value: 'America/Tortola',
    },
    {
        label: '(UTC -04:00) Atlantic/Bermuda',
        value: 'Atlantic/Bermuda',
    },
    {
        label: '(UTC -03:30) America/St_Johns',
        value: 'America/St_Johns',
    },
    {
        label: '(UTC -03:00) America/Araguaina',
        value: 'America/Araguaina',
    },
    {
        label: '(UTC -03:00) America/Argentina/Buenos_Aires',
        value: 'America/Argentina/Buenos_Aires',
    },
    {
        label: '(UTC -03:00) America/Argentina/Catamarca',
        value: 'America/Argentina/Catamarca',
    },
    {
        label: '(UTC -03:00) America/Argentina/Cordoba',
        value: 'America/Argentina/Cordoba',
    },
    {
        label: '(UTC -03:00) America/Argentina/Jujuy',
        value: 'America/Argentina/Jujuy',
    },
    {
        label: '(UTC -03:00) America/Argentina/La_Rioja',
        value: 'America/Argentina/La_Rioja',
    },
    {
        label: '(UTC -03:00) America/Argentina/Mendoza',
        value: 'America/Argentina/Mendoza',
    },
    {
        label: '(UTC -03:00) America/Argentina/Rio_Gallegos',
        value: 'America/Argentina/Rio_Gallegos',
    },
    {
        label: '(UTC -03:00) America/Argentina/Salta',
        value: 'America/Argentina/Salta',
    },
    {
        label: '(UTC -03:00) America/Argentina/San_Juan',
        value: 'America/Argentina/San_Juan',
    },
    {
        label: '(UTC -03:00) America/Argentina/San_Luis',
        value: 'America/Argentina/San_Luis',
    },
    {
        label: '(UTC -03:00) America/Argentina/Tucuman',
        value: 'America/Argentina/Tucuman',
    },
    {
        label: '(UTC -03:00) America/Argentina/Ushuaia',
        value: 'America/Argentina/Ushuaia',
    },
    {
        label: '(UTC -03:00) America/Asuncion',
        value: 'America/Asuncion',
    },
    {
        label: '(UTC -03:00) America/Bahia',
        value: 'America/Bahia',
    },
    {
        label: '(UTC -03:00) America/Belem',
        value: 'America/Belem',
    },
    {
        label: '(UTC -03:00) America/Campo_Grande',
        value: 'America/Campo_Grande',
    },
    {
        label: '(UTC -03:00) America/Cayenne',
        value: 'America/Cayenne',
    },
    {
        label: '(UTC -03:00) America/Cuiaba',
        value: 'America/Cuiaba',
    },
    {
        label: '(UTC -03:00) America/Fortaleza',
        value: 'America/Fortaleza',
    },
    {
        label: '(UTC -03:00) America/Godthab',
        value: 'America/Godthab',
    },
    {
        label: '(UTC -03:00) America/Maceio',
        value: 'America/Maceio',
    },
    {
        label: '(UTC -03:00) America/Miquelon',
        value: 'America/Miquelon',
    },
    {
        label: '(UTC -03:00) America/Montevideo',
        value: 'America/Montevideo',
    },
    {
        label: '(UTC -03:00) America/Paramaribo',
        value: 'America/Paramaribo',
    },
    {
        label: '(UTC -03:00) America/Punta_Arenas',
        value: 'America/Punta_Arenas',
    },
    {
        label: '(UTC -03:00) America/Recife',
        value: 'America/Recife',
    },
    {
        label: '(UTC -03:00) America/Santarem',
        value: 'America/Santarem',
    },
    {
        label: '(UTC -03:00) America/Santiago',
        value: 'America/Santiago',
    },
    {
        label: '(UTC -03:00) Antarctica/Palmer',
        value: 'Antarctica/Palmer',
    },
    {
        label: '(UTC -03:00) Antarctica/Rothera',
        value: 'Antarctica/Rothera',
    },
    {
        label: '(UTC -03:00) Atlantic/Stanley',
        value: 'Atlantic/Stanley',
    },
    {
        label: '(UTC -02:00) America/Noronha',
        value: 'America/Noronha',
    },
    {
        label: '(UTC -02:00) America/Sao_Paulo',
        value: 'America/Sao_Paulo',
    },
    {
        label: '(UTC -02:00) Atlantic/South_Georgia',
        value: 'Atlantic/South_Georgia',
    },
    {
        label: '(UTC -01:00) America/Scoresbysund',
        value: 'America/Scoresbysund',
    },
    {
        label: '(UTC -01:00) Atlantic/Azores',
        value: 'Atlantic/Azores',
    },
    {
        label: '(UTC -01:00) Atlantic/Cape_Verde',
        value: 'Atlantic/Cape_Verde',
    },
    {
        label: '(UTC -00:00) Africa/Abidjan',
        value: 'Africa/Abidjan',
    },
    {
        label: '(UTC -00:00) Africa/Accra',
        value: 'Africa/Accra',
    },
    {
        label: '(UTC -00:00) Africa/Bamako',
        value: 'Africa/Bamako',
    },
    {
        label: '(UTC -00:00) Africa/Banjul',
        value: 'Africa/Banjul',
    },
    {
        label: '(UTC -00:00) Africa/Bissau',
        value: 'Africa/Bissau',
    },
    {
        label: '(UTC -00:00) Africa/Casablanca',
        value: 'Africa/Casablanca',
    },
    {
        label: '(UTC -00:00) Africa/Conakry',
        value: 'Africa/Conakry',
    },
    {
        label: '(UTC -00:00) Africa/Dakar',
        value: 'Africa/Dakar',
    },
    {
        label: '(UTC -00:00) Africa/El_Aaiun',
        value: 'Africa/El_Aaiun',
    },
    {
        label: '(UTC -00:00) Africa/Freetown',
        value: 'Africa/Freetown',
    },
    {
        label: '(UTC -00:00) Africa/Lome',
        value: 'Africa/Lome',
    },
    {
        label: '(UTC -00:00) Africa/Monrovia',
        value: 'Africa/Monrovia',
    },
    {
        label: '(UTC -00:00) Africa/Nouakchott',
        value: 'Africa/Nouakchott',
    },
    {
        label: '(UTC -00:00) Africa/Ouagadougou',
        value: 'Africa/Ouagadougou',
    },
    {
        label: '(UTC -00:00) Africa/Sao_Tome',
        value: 'Africa/Sao_Tome',
    },
    {
        label: '(UTC -00:00) America/Danmarkshavn',
        value: 'America/Danmarkshavn',
    },
    {
        label: '(UTC -00:00) Antarctica/Troll',
        value: 'Antarctica/Troll',
    },
    {
        label: '(UTC -00:00) Atlantic/Canary',
        value: 'Atlantic/Canary',
    },
    {
        label: '(UTC -00:00) Atlantic/Faroe',
        value: 'Atlantic/Faroe',
    },
    {
        label: '(UTC -00:00) Atlantic/Madeira',
        value: 'Atlantic/Madeira',
    },
    {
        label: '(UTC -00:00) Atlantic/Reykjavik',
        value: 'Atlantic/Reykjavik',
    },
    {
        label: '(UTC +00:00) Europe/Lisbon',
        value: 'Europe/Lisbon',
    },
    {
        label: '(UTC +01:00) Africa/Algiers',
        value: 'Africa/Algiers',
    },
    {
        label: '(UTC +01:00) Africa/Bangui',
        value: 'Africa/Bangui',
    },
    {
        label: '(UTC +01:00) Africa/Brazzaville',
        value: 'Africa/Brazzaville',
    },
    {
        label: '(UTC +01:00) Africa/Ceuta',
        value: 'Africa/Ceuta',
    },
    {
        label: '(UTC +01:00) Africa/Douala',
        value: 'Africa/Douala',
    },
    {
        label: '(UTC +01:00) Africa/Kinshasa',
        value: 'Africa/Kinshasa',
    },
    {
        label: '(UTC +01:00) Africa/Lagos',
        value: 'Africa/Lagos',
    },
    {
        label: '(UTC +01:00) Africa/Libreville',
        value: 'Africa/Libreville',
    },
    {
        label: '(UTC +01:00) Africa/Luanda',
        value: 'Africa/Luanda',
    },
    {
        label: '(UTC +01:00) Africa/Malabo',
        value: 'Africa/Malabo',
    },
    {
        label: '(UTC +01:00) Africa/Ndjamena',
        value: 'Africa/Ndjamena',
    },
    {
        label: '(UTC +01:00) Africa/Niamey',
        value: 'Africa/Niamey',
    },
    {
        label: '(UTC +01:00) Africa/Porto-Novo',
        value: 'Africa/Porto-Novo',
    },
    {
        label: '(UTC +01:00) Africa/Tunis',
        value: 'Africa/Tunis',
    },
    {
        label: '(UTC +01:00) Arctic/Longyearbyen',
        value: 'Arctic/Longyearbyen',
    },
    {
        label: '(UTC +01:00) Europe/Amsterdam',
        value: 'Europe/Amsterdam',
    },
    {
        label: '(UTC +01:00) Europe/Andorra',
        value: 'Europe/Andorra',
    },
    {
        label: '(UTC +01:00) Europe/Belgrade',
        value: 'Europe/Belgrade',
    },
    {
        label: '(UTC +01:00) Europe/Berlin',
        value: 'Europe/Berlin',
    },
    {
        label: '(UTC +01:00) Europe/Bratislava',
        value: 'Europe/Bratislava',
    },
    {
        label: '(UTC +01:00) Europe/Brussels',
        value: 'Europe/Brussels',
    },
    {
        label: '(UTC +01:00) Europe/Budapest',
        value: 'Europe/Budapest',
    },
    {
        label: '(UTC +01:00) Europe/Busingen',
        value: 'Europe/Busingen',
    },
    {
        label: '(UTC +01:00) Europe/Copenhagen',
        value: 'Europe/Copenhagen',
    },
    {
        label: '(UTC +01:00) Europe/Gibraltar',
        value: 'Europe/Gibraltar',
    },
    {
        label: '(UTC +01:00) Europe/Ljubljana',
        value: 'Europe/Ljubljana',
    },
    {
        label: '(UTC +01:00) Europe/Luxembourg',
        value: 'Europe/Luxembourg',
    },
    {
        label: '(UTC +01:00) Europe/Madrid',
        value: 'Europe/Madrid',
    },
    {
        label: '(UTC +01:00) Europe/Malta',
        value: 'Europe/Malta',
    },
    {
        label: '(UTC +01:00) Europe/Monaco',
        value: 'Europe/Monaco',
    },
    {
        label: '(UTC +01:00) Europe/Oslo',
        value: 'Europe/Oslo',
    },
    {
        label: '(UTC +01:00) Europe/Paris',
        value: 'Europe/Paris',
    },
    {
        label: '(UTC +01:00) Europe/Podgorica',
        value: 'Europe/Podgorica',
    },
    {
        label: '(UTC +01:00) Europe/Prague',
        value: 'Europe/Prague',
    },
    {
        label: '(UTC +01:00) Europe/Rome',
        value: 'Europe/Rome',
    },
    {
        label: '(UTC +01:00) Europe/San_Marino',
        value: 'Europe/San_Marino',
    },
    {
        label: '(UTC +01:00) Europe/Sarajevo',
        value: 'Europe/Sarajevo',
    },
    {
        label: '(UTC +01:00) Europe/Skopje',
        value: 'Europe/Skopje',
    },
    {
        label: '(UTC +01:00) Europe/Stockholm',
        value: 'Europe/Stockholm',
    },
    {
        label: '(UTC +01:00) Europe/Tirane',
        value: 'Europe/Tirane',
    },
    {
        label: '(UTC +01:00) Europe/Vaduz',
        value: 'Europe/Vaduz',
    },
    {
        label: '(UTC +01:00) Europe/Vatican',
        value: 'Europe/Vatican',
    },
    {
        label: '(UTC +01:00) Europe/Vienna',
        value: 'Europe/Vienna',
    },
    {
        label: '(UTC +01:00) Europe/Warsaw',
        value: 'Europe/Warsaw',
    },
    {
        label: '(UTC +01:00) Europe/Zagreb',
        value: 'Europe/Zagreb',
    },
    {
        label: '(UTC +01:00) Europe/Zurich',
        value: 'Europe/Zurich',
    },
    {
        label: '(UTC +02:00) Africa/Blantyre',
        value: 'Africa/Blantyre',
    },
    {
        label: '(UTC +02:00) Africa/Bujumbura',
        value: 'Africa/Bujumbura',
    },
    {
        label: '(UTC +02:00) Africa/Cairo',
        value: 'Africa/Cairo',
    },
    {
        label: '(UTC +02:00) Africa/Gaborone',
        value: 'Africa/Gaborone',
    },
    {
        label: '(UTC +02:00) Africa/Harare',
        value: 'Africa/Harare',
    },
    {
        label: '(UTC +02:00) Africa/Johannesburg',
        value: 'Africa/Johannesburg',
    },
    {
        label: '(UTC +02:00) Africa/Kigali',
        value: 'Africa/Kigali',
    },
    {
        label: '(UTC +02:00) Africa/Lubumbashi',
        value: 'Africa/Lubumbashi',
    },
    {
        label: '(UTC +02:00) Africa/Lusaka',
        value: 'Africa/Lusaka',
    },
    {
        label: '(UTC +02:00) Africa/Maputo',
        value: 'Africa/Maputo',
    },
    {
        label: '(UTC +02:00) Africa/Maseru',
        value: 'Africa/Maseru',
    },
    {
        label: '(UTC +02:00) Africa/Mbabane',
        value: 'Africa/Mbabane',
    },
    {
        label: '(UTC +02:00) Africa/Tripoli',
        value: 'Africa/Tripoli',
    },
    {
        label: '(UTC +02:00) Africa/Windhoek',
        value: 'Africa/Windhoek',
    },
    {
        label: '(UTC +02:00) Asia/Amman',
        value: 'Asia/Amman',
    },
    {
        label: '(UTC +02:00) Asia/Beirut',
        value: 'Asia/Beirut',
    },
    {
        label: '(UTC +02:00) Asia/Damascus',
        value: 'Asia/Damascus',
    },
    { label: '(UTC +02:00) Asia/Gaza', value: 'Asia/Gaza' },
    { label: '(UTC +02:00) Asia/Hebron', value: 'Asia/Hebron' },
    { label: '(UTC +02:00) Asia/Jerusalem', value: 'Asia/Jerusalem' },
    { label: '(UTC +02:00) Asia/Nicosia', value: 'Asia/Nicosia' },
    { label: '(UTC +02:00) Europe/Athens', value: 'Europe/Athens' },
    { label: '(UTC +02:00) Europe/Bucharest', value: 'Europe/Bucharest' },
    { label: '(UTC +02:00) Europe/Chisinau', value: 'Europe/Chisinau' },
    { label: '(UTC +02:00) Europe/Helsinki', value: 'Europe/Helsinki' },
    { label: '(UTC +02:00) Europe/Kaliningrad', value: 'Europe/Kaliningrad' },
    { label: '(UTC +02:00) Europe/Kiev', value: 'Europe/Kiev' },
    { label: '(UTC +02:00) Europe/Mariehamn', value: 'Europe/Mariehamn' },
    { label: '(UTC +02:00) Europe/Riga', value: 'Europe/Riga' },
    { label: '(UTC +02:00) Europe/Sofia', value: 'Europe/Sofia' },
    { label: '(UTC +02:00) Europe/Tallinn', value: 'Europe/Tallinn' },
    { label: '(UTC +02:00) Europe/Uzhgorod', value: 'Europe/Uzhgorod' },
    { label: '(UTC +02:00) Europe/Vilnius', value: 'Europe/Vilnius' },
    { label: '(UTC +02:00) Europe/Zaporozhye', value: 'Europe/Zaporozhye' },
    { label: '(UTC +03:00) Africa/Addis_Ababa', value: 'Africa/Addis_Ababa' },
    { label: '(UTC +03:00) Africa/Asmara', value: 'Africa/Asmara' },
    {
        label: '(UTC +03:00) Africa/Dar_es_Salaam',
        value: 'Africa/Dar_es_Salaam',
    },
    { label: '(UTC +03:00) Africa/Djibouti', value: 'Africa/Djibouti' },
    { label: '(UTC +03:00) Africa/Juba', value: 'Africa/Juba' },
    { label: '(UTC +03:00) Africa/Kampala', value: 'Africa/Kampala' },
    { label: '(UTC +03:00) Africa/Khartoum', value: 'Africa/Khartoum' },
    { label: '(UTC +03:00) Africa/Mogadishu', value: 'Africa/Mogadishu' },
    { label: '(UTC +03:00) Africa/Nairobi', value: 'Africa/Nairobi' },
    { label: '(UTC +03:00) Antarctica/Syowa', value: 'Antarctica/Syowa' },
    { label: '(UTC +03:00) Asia/Aden', value: 'Asia/Aden' },
    { label: '(UTC +03:00) Asia/Baghdad', value: 'Asia/Baghdad' },
    { label: '(UTC +03:00) Asia/Bahrain', value: 'Asia/Bahrain' },
    { label: '(UTC +03:00) Asia/Famagusta', value: 'Asia/Famagusta' },
    { label: '(UTC +03:00) Asia/Kuwait', value: 'Asia/Kuwait' },
    { label: '(UTC +03:00) Asia/Qatar', value: 'Asia/Qatar' },
    { label: '(UTC +03:00) Asia/Riyadh', value: 'Asia/Riyadh' },
    { label: '(UTC +03:00) Europe/Istanbul', value: 'Europe/Istanbul' },
    { label: '(UTC +03:00) Europe/Kirov', value: 'Europe/Kirov' },
    { label: '(UTC +03:00) Europe/Minsk', value: 'Europe/Minsk' },
    { label: '(UTC +03:00) Europe/Moscow', value: 'Europe/Moscow' },
    { label: '(UTC +03:00) Europe/Simferopol', value: 'Europe/Simferopol' },
    { label: '(UTC +03:00) Europe/Volgograd', value: 'Europe/Volgograd' },
    { label: '(UTC +03:00) Indian/Antananarivo', value: 'Indian/Antananarivo' },
    { label: '(UTC +03:00) Indian/Comoro', value: 'Indian/Comoro' },
    { label: '(UTC +03:00) Indian/Mayotte', value: 'Indian/Mayotte' },
    { label: '(UTC +03:30) Asia/Tehran', value: 'Asia/Tehran' },
    { label: '(UTC +04:00) Asia/Baku', value: 'Asia/Baku' },
    { label: '(UTC +04:00) Asia/Dubai', value: 'Asia/Dubai' },
    { label: '(UTC +04:00) Asia/Muscat', value: 'Asia/Muscat' },
    { label: '(UTC +04:00) Asia/Tbilisi', value: 'Asia/Tbilisi' },
    { label: '(UTC +04:00) Asia/Yerevan', value: 'Asia/Yerevan' },
    { label: '(UTC +04:00) Europe/Astrakhan', value: 'Europe/Astrakhan' },
    { label: '(UTC +04:00) Europe/Samara', value: 'Europe/Samara' },
    { label: '(UTC +04:00) Europe/Saratov', value: 'Europe/Saratov' },
    { label: '(UTC +04:00) Europe/Ulyanovsk', value: 'Europe/Ulyanovsk' },
    { label: '(UTC +04:00) Indian/Mahe', value: 'Indian/Mahe' },
    { label: '(UTC +04:00) Indian/Mauritius', value: 'Indian/Mauritius' },
    { label: '(UTC +04:00) Indian/Reunion', value: 'Indian/Reunion' },
    { label: '(UTC +04:30) Asia/Kabul', value: 'Asia/Kabul' },
    { label: '(UTC +05:00) Asia/Karachi', value: 'Asia/Karachi' },
    { label: '(UTC +05:30) Asia/Colombo', value: 'Asia/Colombo' },
    { label: '(UTC +05:30) Asia/Kolkata', value: 'Asia/Kolkata' },
    { label: '(UTC +05:45) Asia/Kathmandu', value: 'Asia/Kathmandu' },
    { label: '(UTC +06:00) Asia/Dhaka', value: 'Asia/Dhaka' },
    { label: '(UTC +06:00) Asia/Thimphu', value: 'Asia/Thimphu' },
    { label: '(UTC +06:30) Asia/Yangon', value: 'Asia/Yangon' },
    { label: '(UTC +07:00) Asia/Bangkok', value: 'Asia/Bangkok' },
    { label: '(UTC +07:00) Asia/Ho_Chi_Minh', value: 'Asia/Ho_Chi_Minh' },
    { label: '(UTC +07:00) Asia/Jakarta', value: 'Asia/Jakarta' },
    { label: '(UTC +08:00) Asia/Brunei', value: 'Asia/Brunei' },
    { label: '(UTC +08:00) Asia/Choibalsan', value: 'Asia/Choibalsan' },
    { label: '(UTC +08:00) Asia/Hong_Kong', value: 'Asia/Hong_Kong' },
    { label: '(UTC +08:00) Asia/Irkutsk', value: 'Asia/Irkutsk' },
    { label: '(UTC +08:00) Asia/Kuala_Lumpur', value: 'Asia/Kuala_Lumpur' },
    { label: '(UTC +08:00) Asia/Kuching', value: 'Asia/Kuching' },
    { label: '(UTC +08:00) Asia/Macau', value: 'Asia/Macau' },
    { label: '(UTC +08:00) Asia/Makassar', value: 'Asia/Makassar' },
    { label: '(UTC +08:00) Asia/Manila', value: 'Asia/Manila' },
    { label: '(UTC +08:00) Asia/Shanghai', value: 'Asia/Shanghai' },
    { label: '(UTC +08:00) Asia/Singapore', value: 'Asia/Singapore' },
    { label: '(UTC +08:00) Asia/Taipei', value: 'Asia/Taipei' },
    { label: '(UTC +08:00) Asia/Ulaanbaatar', value: 'Asia/Ulaanbaatar' },
    { label: '(UTC +08:00) Australia/Perth', value: 'Australia/Perth' },
    { label: '(UTC +08:30) Asia/Pyongyang', value: 'Asia/Pyongyang' },
    { label: '(UTC +08:45) Australia/Eucla', value: 'Australia/Eucla' },
    { label: '(UTC +09:00) Asia/Chita', value: 'Asia/Chita' },
    { label: '(UTC +09:00) Asia/Dili', value: 'Asia/Dili' },
    { label: '(UTC +09:00) Asia/Jayapura', value: 'Asia/Jayapura' },
    { label: '(UTC +09:00) Asia/Khandyga', value: 'Asia/Khandyga' },
    { label: '(UTC +09:00) Asia/Seoul', value: 'Asia/Seoul' },
    { label: '(UTC +09:00) Asia/Tokyo', value: 'Asia/Tokyo' },
    { label: '(UTC +09:00) Asia/Yakutsk', value: 'Asia/Yakutsk' },
    { label: '(UTC +09:00) Pacific/Palau', value: 'Pacific/Palau' },
    { label: '(UTC +09:30) Australia/Darwin', value: 'Australia/Darwin' },
    {
        label: '(UTC +10:00) Antarctica/DumontDUrville',
        value: 'Antarctica/DumontDUrville',
    },
    { label: '(UTC +10:00) Asia/Ust-Nera', value: 'Asia/Ust-Nera' },
    { label: '(UTC +10:00) Asia/Vladivostok', value: 'Asia/Vladivostok' },
    { label: '(UTC +10:00) Australia/Brisbane', value: 'Australia/Brisbane' },
    { label: '(UTC +10:00) Australia/Lindeman', value: 'Australia/Lindeman' },
    { label: '(UTC +10:00) Pacific/Chuuk', value: 'Pacific/Chuuk' },
    { label: '(UTC +10:00) Pacific/Guam', value: 'Pacific/Guam' },
    {
        label: '(UTC +10:00) Pacific/Port_Moresby',
        value: 'Pacific/Port_Moresby',
    },
    { label: '(UTC +10:00) Pacific/Saipan', value: 'Pacific/Saipan' },
    { label: '(UTC +10:30) Australia/Adelaide', value: 'Australia/Adelaide' },
    {
        label: '(UTC +10:30) Australia/Broken_Hill',
        value: 'Australia/Broken_Hill',
    },
    { label: '(UTC +11:00) Antarctica/Casey', value: 'Antarctica/Casey' },
    {
        label: '(UTC +11:00) Antarctica/Macquarie',
        value: 'Antarctica/Macquarie',
    },
    { label: '(UTC +11:00) Asia/Magadan', value: 'Asia/Magadan' },
    { label: '(UTC +11:00) Asia/Sakhalin', value: 'Asia/Sakhalin' },
    { label: '(UTC +11:00) Asia/Srednekolymsk', value: 'Asia/Srednekolymsk' },
    { label: '(UTC +11:00) Australia/Currie', value: 'Australia/Currie' },
    { label: '(UTC +11:00) Australia/Hobart', value: 'Australia/Hobart' },
    { label: '(UTC +11:00) Australia/Lord_Howe', value: 'Australia/Lord_Howe' },
    { label: '(UTC +11:00) Australia/Melbourne', value: 'Australia/Melbourne' },
    { label: '(UTC +11:00) Australia/Sydney', value: 'Australia/Sydney' },
    {
        label: '(UTC +11:00) Pacific/Bougainville',
        value: 'Pacific/Bougainville',
    },
    { label: '(UTC +11:00) Pacific/Efate', value: 'Pacific/Efate' },
    { label: '(UTC +11:00) Pacific/Guadalcanal', value: 'Pacific/Guadalcanal' },
    { label: '(UTC +11:00) Pacific/Kosrae', value: 'Pacific/Kosrae' },
    { label: '(UTC +11:00) Pacific/Norfolk', value: 'Pacific/Norfolk' },
    { label: '(UTC +11:00) Pacific/Noumea', value: 'Pacific/Noumea' },
    { label: '(UTC +11:00) Pacific/Pohnpei', value: 'Pacific/Pohnpei' },
    { label: '(UTC +12:00) Asia/Anadyr', value: 'Asia/Anadyr' },
    { label: '(UTC +12:00) Asia/Kamchatka', value: 'Asia/Kamchatka' },
    { label: '(UTC +12:00) Pacific/Funafuti', value: 'Pacific/Funafuti' },
    { label: '(UTC +12:00) Pacific/Kwajalein', value: 'Pacific/Kwajalein' },
    { label: '(UTC +12:00) Pacific/Majuro', value: 'Pacific/Majuro' },
    { label: '(UTC +12:00) Pacific/Nauru', value: 'Pacific/Nauru' },
    { label: '(UTC +12:00) Pacific/Tarawa', value: 'Pacific/Tarawa' },
    { label: '(UTC +12:00) Pacific/Wake', value: 'Pacific/Wake' },
    { label: '(UTC +12:00) Pacific/Wallis', value: 'Pacific/Wallis' },
    { label: '(UTC +13:00) Antarctica/McMurdo', value: 'Antarctica/McMurdo' },
    { label: '(UTC +13:00) Pacific/Auckland', value: 'Pacific/Auckland' },
    { label: '(UTC +13:00) Pacific/Enderbury', value: 'Pacific/Enderbury' },
    { label: '(UTC +13:00) Pacific/Fakaofo', value: 'Pacific/Fakaofo' },
    { label: '(UTC +13:00) Pacific/Fiji', value: 'Pacific/Fiji' },
    { label: '(UTC +13:45) Pacific/Chatham', value: 'Pacific/Chatham' },
    { label: '(UTC +14:00) Pacific/Apia', value: 'Pacific/Apia' },
    { label: '(UTC +14:00) Pacific/Kiritimati', value: 'Pacific/Kiritimati' },
    { label: '(UTC +14:00) Pacific/Tongatapu', value: 'Pacific/Tongatapu' },
]
